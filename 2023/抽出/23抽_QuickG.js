function searchDataGQ2023() {
  ...
  if (!sheetB) {
    Logger.log('確認2023(QuickG)が見つかりません: ' + sheetBName);
    SpreadsheetApp.getUi().alert('確認2023(QuickG)が見つかりません: ' + sheetBName);
    return;
  }

  // 確認2023(QuickG)のB2セルの値を取得
  ...
  var startRowB = 7; // 確認2023(QuickG)にデータを反映させる開始行
  for (var r = 0; r < matchingRows.length; r++) {
    ...
    for (var k = 0; k < headersB.length; k++) {
      if (k < 11) { // J列まで
        // 確認2023(QuickG)の5行目のJ列までとG（日経＋QUICK合計）3行目の項目名を比較
        ...
        // 確認2023(QuickG)の5行目のJ列以降とG（日経＋QUICK合計）2行目の項目名を比較
