function checkDataSQ2023(){
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("確認2023(QuickS)");
  ...
  var headerIndices = {
    ...
    startCol: headers.indexOf("【社会】出産・育児休暇後の復職率"),
...
function countColoredCells(sheet) {
  ...
  var conditions = {
    ...
    "種別名": function(value, row) {
      ...
      var startCol = headerIndices.startCol; // 【社会】出産・育児休暇後の復職率
          ...
          for (var col = startCol; col < data[row].length; col++) {
            if (data[row][col] !== "") {
              for (var errorCol = startCol; errorCol < data[row].length; errorCol++) {
                ...
                sheet.getRange(row + 1, 1).setValue("データが不正です");  // A列にエラーメッセージをセット
                sheet.getRange(row + 1, 1).setBackground("orange");
              }
              return false;
            }
