function checkDataGQ2023(){
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("確認2023(QuickG)");
  ...
  var headerIndices = {
    ...
    startCol: headers.indexOf("【ガバナンス】取締役会開催数"),
...
function countColoredCells(sheet) {
  ...
  var conditions = {
    ...
    "種別名": function(value, row) {
      ...
      var startCol = headerIndices.startCol; // 【ガバナンス】取締役会開催数
  ...
  for (var row = 6; row < data.length; row++) {
      ...
      for (var col = headerIndices.startCol; col < data[row].length; col++) {
        if (data[row][col] !== "") {
          ...
          for (var row2 = 6; row2 < data.length; row2++) {
            ...
            var exclusionData = (headerIndices.environmentalReserveCol || headers.indexOf("【ガバナンス】取締役会の議長"));
              if (exclusionData){
                // 加工データのチェックは行う
    ...
    var thresholds = {
      "開示データ": {
        "【ガバナンス】取締役会開催数": { "min": 0, "max": 45 },
        "【ガバナンス】女性取締役数": { "min": 0, "max": 50 },
        "【ガバナンス】女性取締役比率": { "min": 0, "max": 100 },
        "【ガバナンス】取締役会への出席率": { "min": 0, "max": 100 },
        "【ガバナンス】女性監査役の人数": { "min": 0, "max": 100 },
        "【ガバナンス】監査役会への出席率": { "min": 0, "max": 100 },
        "【ガバナンス】取締役平均年齢": { "min": 0, "max": 100 },
      },
      "加工データ": {
        "【ガバナンス】取締役会開催数": { "min": 0, "max": 45 },
        "【ガバナンス】女性取締役数": { "min": 0, "max": 50 },
