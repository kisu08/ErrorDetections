function searchDataE2021() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();

    // 別のスプレッドシートのIDを指定
  var externalSpreadsheetId = '1ZFRASHrsUZ7VSshC2YP-vvhxZaarRZypAkposZincDM';
  ...
  if (!sheetB) {
    Logger.log('確認2021(日経E)が見つかりません: ' + sheetBName);
    SpreadsheetApp.getUi().alert('確認2021(日経E)が見つかりません: ' + sheetBName);
    return;
  }

  // 確認2021(日経E)のB2セルの値を取得
  ...
  var startRowB = 7; // 確認2021(日経E)にデータを反映させる開始行
  for (var r = 0; r < matchingRows.length; r++) {
    ...
    for (var k = 0; k < headersB.length; k++) {
      if (k < 11) { // J列まで
        // 確認2021(日経E)の5行目のJ列までとE（日経＋QUICK合計）3行目の項目名を比較
        ...
        // 確認2021(日経E)の5行目のJ列以降とE（日経＋QUICK合計）2行目の項目名を比較
