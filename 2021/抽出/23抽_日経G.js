function searchDataG2021() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();

    // 別のスプレッドシートのIDを指定
  var externalSpreadsheetId = '17tCgB2WKoq9NaEtNW5oMbyYp3za6a0dam8Oqifb2V5g';
  ...
  if (!sheetB) {
    Logger.log('確認2021(日経G)が見つかりません: ' + sheetBName);
    SpreadsheetApp.getUi().alert('確認2021(日経G)が見つかりません: ' + sheetBName);
    return;
  }

  // 確認2021(日経G)のB2セルの値を取得
  ...
  var startRowB = 7; // 確認2021(日経G)にデータを反映させる開始行
  for (var r = 0; r < matchingRows.length; r++) {
    ...
    for (var j = 0; j < headersB.length; j++) {
      if (j < 10) { // J列まで
        // 確認2021(日経G)の5行目のJ列までとG（日経＋QUICK合計）3行目の項目名を比較
        ...
        // 確認2021(日経G)の5行目のJ列以降とG（日経＋QUICK合計）2行目の項目名を比較
