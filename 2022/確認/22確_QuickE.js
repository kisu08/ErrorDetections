function checkDataEQ2022(){
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("確認2022(QuickE)");
  ...
  var conditions = {
    ...
    "過年度：年": function(value, row) {
      ...
      // 数字の打ち間違いで異常に大きいまたは小さい年を検出（2000～2023年の範囲内）
    ...
    "過年度：年月／単位（加工値）": function(value, row) {
      ...
      // 年月の年部分が2000～2023年の範囲内であること
      var yearMonth = parseInt(valueStr.slice(0, 4), 10);
      if (yearMonth < 2000 || yearMonth > 2023) {
        return false;
      }
