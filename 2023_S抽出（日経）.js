function searchDataS2023() {
  ...
  if (!sheetB) {
    Logger.log('確認2023(日経S)が見つかりません: ' + sheetBName);
    SpreadsheetApp.getUi().alert('確認2023(日経S)が見つかりません: ' + sheetBName);
    return;
  }

  // 確認2023(日経S)のB2セルの値を取得
  ...
  var startRowB = 7; // 確認2023(日経S)にデータを反映させる開始行
  for (var r = 0; r < matchingRows.length; r++) {
    ...
    for (var j = 0; j < headersB.length; j++) {
      if (j < 10) { // J列まで
        // 確認2023(日経S)の5行目のJ列までとS（日経＋QUICK合計）3行目の項目名を比較
        ...
        // 確認2023(日経S)の5行目のJ列以降とS（日経＋QUICK合計）2行目の項目名を比較
